...
# Filtering employees with two or more jobs
multi_job_employee_ids = df.groupby('EMPLOYEE_ID').filter(lambda x: len(x) >= 2)['EMPLOYEE_ID'].unique()

print(multi_job_employee_ids)
